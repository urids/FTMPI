...
dnl 
dnl Generated by uriel at Wed Feb  4 19:26:20 2015
dnl on myHost.
dnl ---------------------------------------------------------------------------
...
m4_define([mca_opal_framework_list], [common, backtrace, compress, crs, db, event, hwloc, if, installdirs, memchecker, memcpy, memory, pstat, sec, shmem, timer])
...
m4_define([mca_opal_backtrace_m4_config_component_list], [execinfo, printstack, none])
...
m4_define([mca_opal_crs_m4_config_component_list], [self, dmtcp, blcr])
m4_define([mca_opal_crs_no_config_component_list], [none])

dnl Components in the opal / db framework
m4_define([mca_opal_db_m4_config_component_list], [sqlite, pmi])
...
m4_define([mca_opal_hwloc_m4_config_component_list], [external, hwloc172])
m4_define([mca_opal_hwloc_no_config_component_list], [])

dnl Components in the opal / if framework
m4_define([mca_opal_if_m4_config_component_list], [posix_ipv4, bsdx_ipv6, linux_ipv6, solaris_ipv6, bsdx_ipv4])
...
m4_define([mca_opal_timer_m4_config_component_list], [solaris, darwin, aix, linux, altix, catamount])
...
m4_define([mca_orte_errmgr_no_config_component_list], [default_app, default_hnp, default_tool, default_orted])

dnl Components in the orte / ess framework
m4_define([mca_orte_ess_m4_config_component_list], [pmi, tm, lsf, slurm, alps])
m4_define([mca_orte_ess_no_config_component_list], [hnp, singleton, tool, env])
...
m4_define([mca_orte_plm_m4_config_component_list], [isolated, tm, rsh, lsf, slurm, alps])
m4_define([mca_orte_plm_no_config_component_list], [])

dnl Components in the orte / ras framework
m4_define([mca_orte_ras_m4_config_component_list], [gridengine, tm, lsf, loadleveler, slurm, alps])
m4_define([mca_orte_ras_no_config_component_list], [simulator])

dnl Components in the orte / rmaps framework
m4_define([mca_orte_rmaps_m4_config_component_list], [mindist, rank_file, lama])
m4_define([mca_orte_rmaps_no_config_component_list], [round_robin, staged, ppr, resilient, seq])
...
m4_define([mca_orte_routed_no_config_component_list], [binomial, direct, debruijn, radix])

dnl Components in the orte / sensor framework
m4_define([mca_orte_sensor_m4_config_component_list], [resusage, file, ft_tester, heartbeat])
...
m4_define([mca_orte_state_no_config_component_list], [hnp, staged_orted, app, orted, staged_hnp, tool, novm])
...
m4_define([mca_ompi_common_m4_config_component_list], [ugni, sm, verbs, cuda, mx])
...
m4_define([mca_ompi_btl_m4_config_component_list], [ugni, tcp, openib, scif, sm, vader, usnic, smcuda, portals4])
m4_define([mca_ompi_btl_no_config_component_list], [self])

dnl Components in the ompi / coll framework
m4_define([mca_ompi_coll_m4_config_component_list], [hierarch, hcoll, portals4, fca])
m4_define([mca_ompi_coll_no_config_component_list], [inter, basic, sm, self, ml, libnbc, tuned])
...
m4_define([mca_ompi_fcoll_no_config_component_list], [static, two_phase, individual, dynamic, ylib])
...
m4_define([mca_ompi_mpool_no_config_component_list], [sm, grdma])

dnl Components in the ompi / mtl framework
m4_define([mca_ompi_mtl_m4_config_component_list], [psm, portals4, mxm, mx])
...
m4_define([mca_ompi_osc_m4_config_component_list], [portals4, rdma])
m4_define([mca_ompi_osc_no_config_component_list], [sm])

dnl Components in the ompi / pml framework
m4_define([mca_ompi_pml_m4_config_component_list], [crcpw, ob1, v, bfo])
...
m4_define([mca_ompi_sharedfp_no_config_component_list], [sm, lockedfile, individual])
...
m4_define([mca_oshmem_sshmem_m4_config_component_list], [verbs, sysv, mmap])
...
m4_include([opal/mca/hwloc/hwloc172/configure.m4])
...
m4_define([ompi_mpiext_list], [affinity, FTFrame, cr])

dnl List of configure.m4 files to include
m4_include([ompi/mpiext/affinity/configure.m4])
m4_include([ompi/mpiext/FTFrame/configure.m4])
