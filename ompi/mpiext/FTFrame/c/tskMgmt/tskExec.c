...
//#define OMPI_XclExecKernel(dataFilePath , fmt, ...) _OMPI_XclExecKernel(dataFilePath , fmt,  ##__VA_ARGS__)
 ...
 int XclExecKernel(MPI_Comm comm, int globalThreads, int localThreads, const char * fmt, va_list arg) {
	...
	dlhandle =dlopen("/home/uriel/Dev/mpiApps/FTWrkDistr/multiDeviceMgmt/Debug/libmultiDeviceMgmt.so",
					RTLD_LAZY);
...
/*
	...
	char *error;
		...
		int numbuffers=0;
		for(j=0;j<entityData.blockCount;j++){
			numbuffers+=entityData.blocklen[j];
		}

		int* bufferSize;
		bufferSize=(int*)malloc(numbuffers*sizeof(int));

		int acum=0;
		for(j=0;j<entityData.blockCount;j++){
			for(k=0;k<entityData.blocklen[j];k++){
				bufferSize[acum+k]=sizeof(entityData.basictypes[j])*entitiesPerRank;
			}
			acum+=entityData.blocklen[j];
		}

			err=(*createBuffers)(numTasks,numConsts,bufferSize);
*/
  return 0;
